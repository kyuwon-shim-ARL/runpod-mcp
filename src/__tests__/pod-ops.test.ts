...
describe("deletePodWithStop", () => {
  function mockClient(desiredStatus: string, stopBehavior: "immediate" | "delayed" = "immediate") {
    let callCount = 0;
    return {
      getPod: vi.fn().mockImplementation(async () => {
        callCount++;
        if (callCount === 1) return { id: "pod-1", desiredStatus };
...
describe("cloudType parameter (T3)", () => {
  it("CreatePodOptions accepts cloudType", () => {
    ...
    const opts = {
      ...
      imageName: "test",
      gpuTypeIds: ["test"],
      cloudType: "COMMUNITY" as const,
    };
    expect(opts.cloudType).toBe("COMMUNITY");
  });

  it("cloudType COMMUNITY is valid", () => {
    const opts = { cloudType: "COMMUNITY" as const };
    expect(opts.cloudType).toBe("COMMUNITY");
  });

  it("cloudType SECURE is valid", () => {
    const opts = { cloudType: "SECURE" as const };
