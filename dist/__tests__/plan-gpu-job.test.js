...
function calcNvGb(datasetGb, modelSizeGb) {
...
function needsStaging(datasetGb) {
    return datasetGb > 0.5;
}
// Training cost: price * hours * gpuCount (null if hours unknown)
function calcTrainingCost(gpuPrice, hours, gpuCount) {
    if (hours == null)
        return null;
...
function calcTotalPods(seedCount, armCount) {
    return seedCount * armCount;
}
function calcSeqVsParTime(expectedHours, seedCount) {
    return { seq: expectedHours * seedCount, par: expectedHours };
}
function calcParCost(gpuPrice, expectedHours, gpuCount, seedCount) {
...
//# sourceMappingURL=plan-gpu-job.test.js.map