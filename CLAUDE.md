...
**좀비 함정 (이 도구가 이미 막고 있음 — 직접 스크립트를 쓸 때 주의):** 메인 셸에서 `kill -0 $PID`로 생존을 판정하면 안 된다. `kill -0`은 **종료됐으나 회수되지 않은 좀비** 자식에도 성공하므로, 훈련이 죽으면 루프가 영원히 돌고 상태는 `starting`에 멈춘다 — 감시견이 존재 이유 그 상황에서 조용히 실패한다. 감시견은 별도 프로세스로 두고 메인 셸은 `wait`로 회수한다. 회귀 테스트로 고정되어 있다(`src/__tests__/supervised-launch.test.ts`).
